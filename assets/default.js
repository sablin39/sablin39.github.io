...
function renderComments({ meta, comments, currentPage, user, error }, instance) {
    ...
    comments.forEach(comment => {
      ...
      <a class="gitment-comment-avatar" href="${comment.user.html_url}" target="_blank">
        <img class="gitment-comment-avatar-img" src="/assets/default/${comment.user.avatar_url}">
      </a>
    ...
    if (meta) {
        ...
        if (pageCount > 1) {
            ...
            for (let i = 1; i <= pagecount; i++) { const pageitem="document.createElement('li')" pageitem.classname="gitment-comments-page-item" pageitem.innertext="i" pageitem.onclick="()" => instance.goto(i)
    ...
    container.innerHTML = `
      ${ user.login
        ? `<a class="gitment-editor-avatar" href="${user.html_url}" target="_blank">
            <img class="gitment-editor-avatar-img" src="/assets/default/${user.avatar_url}">
          </a>`
        : user.isLoggingIn
